        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-10">
          ...
          <div className="flex flex-col items-start gap-8 max-w-sm shrink-0">
            ...
            <a href="./blog" className="flex items-center gap-3 group">
              <div className="text-[#476D07] w-5 h-5 flex items-center justify-center transition-transform duration-300 group-hover:translate-x-1 group-hover:translate-y-1">
...
const ArticleCard = ({ post, isMain = false }: { post: any, isMain?: boolean }) => (
  <a href={post.link} className={`group flex flex-col h-full ${isMain ? 'gap-8' : 'gap-6'} cursor-pointer`}>
    ...
    <div className="relative w-full overflow-hidden bg-gray-100 aspect-[16/10]">
      ...
      <div className="absolute top-6 right-6 z-10 w-4 h-4 border-b-[2px] border-r-[2px] border-[#588B00] transform -rotate-90"></div>
    </div>
    ...
    <div className="flex flex-col flex-grow items-start justify-between gap-6">
      ...
      <div className="mt-auto">
        <div className="px-5 py-2.5 border border-[#E5E5E5] rounded-full bg-white group-hover:bg-[#AFFF00] group-hover:border-[#AFFF00] transition-colors duration-300">
          <span className="text-[11px] font-semibold tracking-wide uppercase text-[#0C0C0C] group-hover:text-black transition-colors duration-300">
            Read more
          </span>
        </div>
      </div>
