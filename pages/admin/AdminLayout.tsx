...
const navItems: { key: ActivePage; label: string; path: string; icon: React.ReactNode }[] = [
  {
  ...
  {
    key: 'posts', label: 'Blog Posts', path: '/admin/posts',
  ...
  {
    key: 'contacts', label: 'Contact Forms', path: '/admin/contacts',
  ...
  {
    key: 'docs', label: 'Docs', path: '/admin/docs',
...
const AdminLayout: React.FC<{ children: React.ReactNode; active: ActivePage }> = ({ children, active }) => {
  ...
  const SidebarContent = () => (
    <>
      ...
      <nav className="flex-1 px-3 py-5 flex flex-col gap-0.5">
        {navItems.map(item => (
          <Link key={item.key} to={item.path} onClick={() => setMobileOpen(false)}
            className={`flex items-center gap-3 px-3 py-2.5 text-[13px] transition-all duration-200 rounded-sm
              ${active === item.key
            ...
            {item.label}
            {item.key === 'contacts' && (
              <span className="ml-auto text-[9px] font-bold bg-[#AFFF00]/20 text-[#AFFF00]/70 px-1.5 py-0.5">2</span>
            )}
          </Link>
        ))}

        <div className="border-t border-white/[0.05] mt-3 pt-3">
          <Link to="/admin/posts/new" onClick={() => setMobileOpen(false)}
            className="flex items-center gap-3 px-3 py-2.5 text-[13px] text-[#AFFF00]/50 hover:text-[#AFFF00]/80 hover:bg-[#AFFF00]/[0.04] transition-all duration-200 rounded-sm">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
            New Post
          </Link>
          <Link to="/admin/projects/new" onClick={() => setMobileOpen(false)}
            className="flex items-center gap-3 px-3 py-2.5 text-[13px] text-[#AFFF00]/50 hover:text-[#AFFF00]/80 hover:bg-[#AFFF00]/[0.04] transition-all duration-200 rounded-sm">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
            New Project
          </Link>
      ...
      <div className="px-3 py-5 border-t border-white/[0.05] flex flex-col gap-0.5">
        <Link to="/blog" target="_blank"
          className="flex items-center gap-3 px-3 py-2.5 text-[13px] text-white/20 hover:text-white/50 transition-colors rounded-sm">
        ...
        <button onClick={handleLogout}
          className="flex items-center gap-3 px-3 py-2.5 text-[13px] text-white/20 hover:text-red-400/60 transition-colors w-full rounded-sm">
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white poppins-regular flex selection:bg-[#AFFF00] selection:text-black">
      ...
      {/* Desktop sidebar */}
      <aside className="hidden md:flex fixed top-0 left-0 h-full w-[220px] bg-[#080808] border-r border-white/[0.05] flex-col z-20">
        <SidebarContent />
      </aside>

      {/* Mobile top bar */}
      <div className="md:hidden fixed top-0 left-0 right-0 z-30 bg-[#080808] border-b border-white/[0.05] px-5 py-4 flex items-center justify-between">
        <LogoSVG />
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside
              ...
              className="md:hidden fixed top-0 left-0 h-full w-[260px] bg-[#080808] border-r border-white/[0.05] flex flex-col z-50">
              <SidebarContent />
            </motion.aside>
          </>
        )}
      ...
      {/* Main content */}
      <main className="flex-1 md:ml-[220px] mt-[57px] md:mt-0 overflow-y-auto min-h-screen">
        {children}
      </main>
    </div>
  );
