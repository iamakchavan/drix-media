...
const AdminLayout: React.FC<{ children: React.ReactNode; active: ActivePage }> = ({ children, active }) => {
  ...
  const navItems = [
    ...
    { key: 'contacts' as ActivePage, label: 'Contact Forms', path: '/admin/contacts', badge: unreadCount > 0 ? unreadCount.toString() : undefined, icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg> },
  ...
  const handleLogout = async () => { await supabase.auth.signOut(); sessionStorage.removeItem('admin_auth'); navigate('/admin'); };
